        ...
        <thead>
          <tr>
            {columns.map((col, i) => (
              <th
                key={col}
                className={`sticky top-0 z-20 px-3 py-2 text-left text-xs font-semibold border-b border-r border-gray-200 dark:border-slate-700 whitespace-nowrap ${
                  i === 0
                    ? 'sticky left-0 z-30 bg-gray-100 dark:bg-slate-800 text-gray-700 dark:text-slate-300 min-w-48 border-r-2 border-gray-200 dark:border-slate-700'
                    : mainProducts.includes(col)
                    ? 'bg-sky-50 dark:bg-sky-900/50 text-sky-600 dark:text-sky-300 min-w-40'
                    : 'bg-gray-100 dark:bg-slate-800 text-gray-700 dark:text-slate-300 min-w-40'
                }`}
              >
                {col}
              </th>
                ...
                return (
                  <td
                    ...
                    className={`px-3 py-2 border-r border-gray-100 dark:border-slate-800 align-top ${
                      colIndex === 0
                        ? 'sticky left-0 bg-gray-50 dark:bg-slate-800 font-semibold text-xs z-10 border-r-2 border-gray-200 dark:border-slate-700'
                  ...
                  >
                    {colIndex === 0 ? (
                      <span className="uppercase tracking-wide text-[10px] font-semibold text-gray-500 dark:text-slate-400">{cellValue}</span>
                    ) : (
                      <div>
